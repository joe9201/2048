...
def encode_state(mat, max_tile_value=11):
    """
    Converts the game into a 2D one-hot encoded representation
    
    Args:
        mat: the 2D list representing the grid
        max_tile_value: the maximum possible tile value in the game
    Returns:
        A 2D list representing the one-hot encoded state
    """
    encoded_grid = []
    for row in mat:
        row_encoding = []
        for value in row:
            one_hot = [0] * max_tile_value
            if value > 0:
                one_hot[value - 1] = 1
            row_encoding.append(one_hot)
        encoded_grid.append(row_encoding)
    return encoded_grid
...
def main_human_player():
    ...
    while True:
        ...
        # Before performing the move, print the intended direction:
        print("Player move:", direction_map[command])  # Use the dictionary for mapping
...
def main_random_agent():
    ...
    while True:
        ...
        print("Random Agent's move:", direction_map[command])  # Print the agent's move
        prev_mat = [row[:] for row in mat]
        prev_score = score
        prev_highest_tile = max([max(row) for row in mat])
