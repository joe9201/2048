...
def calculate_reward(mat, prev_mat, action, prev_score, prev_highest_tile):
    ...
    # Heuristic 1: Reward for creating more empty cells
    num_empty_cells = sum(row.count(0) for row in mat)
    prev_num_empty_cells = sum(row.count(0) for row in prev_mat)
    reward += 0.1 * (num_empty_cells - prev_num_empty_cells)

    # Heuristic 5: Reward for bringing tiles of the same value closer together
    for i in range(4):
        for j in range(4):
            if mat[i][j] != 0:
                tile_value = mat[i][j]
                for dx, dy in [(1, 0), (0, 1)]:
                    ...
                    if 0 <= x < 4 and 0 <= y < 4 and mat[x][y] == tile_value:
                        ...
                        reward += 0.2  # Reward for bringing tiles closer together

    # Heuristic 6: Reward for placing tiles near the edges or corners
    for i in range(4):
        for j in range(4):
            if mat[i][j] != 0:
                tile_value = mat[i][j]
                ...
                if i == 0 or i == 3 or j == 0 or j == 3:
                    reward += 0.1  # Reward for tiles near the edges
                # Corner occupancy
                if (i == 0 or i == 3) and (j == 0 or j == 3):
                    reward += 0.5  # Reward for tiles in the corners
...
def main_human_player():
    ...
    while True:
        ...
        combined_value = calculate_combined_value(score, reward, weight_score=0.5, weight_reward=0.5)
        print("Reward:", reward)
        print("Score:", score)
